...
import { BudgetExceededError } from '@geosuite/probe-engine'
...
const probeWorker = new Worker<ProbeJob>(PROBE_QUEUE, async (job) => { try { await executeProbeJob(job.data, deps.probe) } catch (error) { const retryable = error instanceof EngineError ? error.retryable : false; if (!retryable || job.attemptsMade + 1 >= PROBE_ATTEMPTS) await deps.probe.repos.probeRuns.markJob(job.data.orgId, job.data.runId, 'failed'); if (!retryable || error instanceof BudgetExceededError) { await job.discard(); } throw error } }, { connection, concurrency: 1 })
